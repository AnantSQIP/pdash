...
// mean, and the day states that are not drawn as segments. Every swatch is drawn exactly as the
// board draws it — pastel fill, the family's edge as a 1px ring, its texture — so the key matches.
//
...
import { type ProjectHue, NO_PROJECT_HUE, FREE_BASE, OVER_COMMITTED, RAIL, PRIORITY_RAMP, segmentRing, textureStyle } from '@/lib/project-colors.clients';
import { cidLabel } from '@/lib/mock-data';

// The four depths with no hue — the same OKLCH lightness steps every client's fills use.
  ...
  return (
    <div className="shrink-0 border-t border-gray-100 bg-gray-50 px-4 py-2.5 text-[11px] text-gray-600 rounded-b-xl">
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
            {projects.map(p => {
              ...
              return (
                ...
                >
                  <span className="h-2.5 w-3.5 shrink-0 rounded-sm" style={{ backgroundColor: hue?.high ?? NO_PROJECT_HUE.high, boxShadow: segmentRing(hue ?? NO_PROJECT_HUE), ...textureStyle(hue?.texture ?? 'none') }} />
            ...
            {hasTeamWork && (
              <span className="inline-flex items-center gap-1.5 px-1.5 py-0.5 text-gray-600" title="Work in a team space — no client, no CID">
                <span className="h-2.5 w-2.5 shrink-0 rounded-sm" style={{ backgroundColor: NO_PROJECT_HUE.high, boxShadow: segmentRing(NO_PROJECT_HUE) }} />Team space
              </span>
            )}
          </div>
        )}
        ...
        {/* Priority depth */}
        <div className="inline-flex items-center gap-1.5" title="Within a client's colour, deeper means more urgent">
          {PRIORITY_RAMP.map((c, i) => (
            <span key={c} className="inline-flex items-center gap-1">
              <span className="h-2.5 w-3.5 rounded-sm" style={{ backgroundColor: c, boxShadow: segmentRing(NO_PROJECT_HUE) }} />
        ...
        <span className="inline-flex items-center gap-1.5">
          <span className="relative h-2.5 w-3.5 rounded-sm" style={{ backgroundColor: NO_PROJECT_HUE.medium, boxShadow: segmentRing(NO_PROJECT_HUE) }}>
        ...
        <span className="inline-flex items-center gap-1.5"><span className="inline-flex h-2.5 w-2.5 items-center justify-center rounded-sm border border-purple-200 bg-purple-50 text-purple-400"><Plane size={8} /></span>Leave</span>
        <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm border border-dashed border-purple-300" />Leave pending</span>
        <span className="inline-flex items-center gap-1.5"><span className="inline-flex h-2.5 w-2.5 items-center justify-center rounded-sm border border-amber-200 bg-amber-50 text-amber-500"><Flag size={8} /></span>Holiday</span>
        <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm border border-slate-100 bg-slate-50" />Weekend</span>
        <span className="inline-flex items-center gap-1.5">
          <span className="relative mb-1 h-2.5 w-3.5 rounded-sm border" style={{ backgroundColor: FREE_BASE.bg, borderColor: FREE_BASE.border }}>
            <span className="absolute inset-x-0 h-[3px] rounded-full" style={{ bottom: -5, backgroundColor: OVER_COMMITTED }} />
          </span>More than 8h that day
        </span>

        <span className="ml-auto text-gray-400">Label inside a bar = the CID's serial · its outline = the client's colour · rose line under a day = more than 8h planned · red rail = that task is late</span>
      </div>
    </div>
  );
}
