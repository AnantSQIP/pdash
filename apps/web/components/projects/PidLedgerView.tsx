...
function exportCsv(rows: PidLedgerEntry[]) {
  // CLIENTS-FLOW: a PID now names a client, so the row that was "Project" is the client. The old
  // "Client" (client code) and "Patents" columns are commented out with those features.
  const perRound = [
    'Client', 'Description', 'Type', 'Phase', 'Priority',
    ...(PATENTS_AND_CLIENT_CODES ? ['Client code'] : []),
    'Progress %',
    'Start', 'End / Deadline', 'Client Deadline',
    'Client Delivered', 'Alloted Hours', 'Logged / Actual Hours', 'PID Total Logged Hours', 'Hours Variance', 'Completed On', 'Closed On',
    ...(PATENTS_AND_CLIENT_CODES ? ['Patents'] : []),
    'Members', 'Created by', 'Client created',
  ];
  const header = [
    'PID', 'PID Status', 'Clients under PID', '#',
    ...perRound,
  ...
  const body = rows.flatMap(r => {
    ...
    if (!rounds.length) {
      // 21 blanks = the per-project columns (Project … Project created). Must track the
      // header: dropping "Working Hours" took this from 22 to 21, and a stale count here
      // silently shifts every later column on reserved-PID rows.
      // One blank per per-round column, COUNTED rather than typed: a hard-coded count here went
      // stale once already and silently shifted every later column on reserved-PID rows.
      return [[r.pid, stateLabel(r.state), 0, '', ...Array(perRound.length).fill(''), r.generatedBy,
    ...
    return rounds.map(rd => [
      ...
      rd.title, rd.description ?? '', rd.type ?? '', rd.phase ?? '', rd.priority ?? '',
      ...(PATENTS_AND_CLIENT_CODES ? [rd.client ?? ''] : []),
      ...
      rd.allottedHours ?? '', rd.actualHours ?? rd.loggedHours ?? 0, r.totalLoggedHours ?? 0,
      // Over/under against what the matter was allotted — the number the ledger is read for.
      rd.allottedHours ? Math.round(((rd.actualHours ?? rd.loggedHours ?? 0) - rd.allottedHours) * 10) / 10 : '',
      rd.completedAt ? formatDateTimeIST(rd.completedAt) : '',
      rd.closedAt ? formatDateTimeIST(rd.closedAt) : '',
      ...(PATENTS_AND_CLIENT_CODES ? [(rd.patents ?? []).join('; ')] : []),
                              ...
                              <div className="space-y-3">
                                {(r.roundCount ?? 0) > 1 && (
                                  <p className="text-[11px] text-gray-500">
                                    ...
                                    {' '}Total logged across all of them: <span className="font-semibold text-gray-700">{r.totalLoggedHours ?? 0}h</span>.
                                  </p>
                                )}
                                {(r.rounds ?? []).map(rd => (
                                  <div key={rd.id} className="rounded-lg border border-gray-200 bg-white p-3">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-x-4 gap-y-1.5">
                                      ...
                                      {/* CLIENTS-FLOW: commented out — the old client code is switched off; the row's title IS the client now. */}
                                      {PATENTS_AND_CLIENT_CODES && <div><span className="text-gray-400">Client code</span><p className="text-gray-800">{rd.client ?? '—'}</p></div>}
                                      ...
                                      {/* Two numbers, not three: what the matter was given, and what it took.
                                      ...
                                      <div>
                                        ...
                                        <p className="text-gray-800">
                                          {rd.actualHours != null ? `${rd.actualHours}h` : `${rd.loggedHours ?? 0}h`}
                                          <span className="text-gray-400 ml-1 text-[11px]">{rd.actualHours != null ? 'actual' : 'logged'}</span>
                                        </p>
                                      </div>
                                    ...
                                    {PATENTS_AND_CLIENT_CODES && (rd.patents?.length ?? 0) > 0 && (
