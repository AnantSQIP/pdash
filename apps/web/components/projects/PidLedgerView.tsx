...
function exportCsv(rows: PidLedgerEntry[]) {
  const header = [
    ...
    'Start', 'Deadline', 'Client Deadline',
  ...
  const body = rows.map(r => [
    ...
    p(r)?.clientDeliveryDate ? formatDateTimeIST(p(r)!.clientDeliveryDate!) : '',
    p(r)?.workingHours ?? '', p(r)?.actualHours ?? '',
    // Actual minus working: the number that says whether the estimate held.
    p(r)?.actualHours != null && p(r)?.workingHours != null
      ? Math.round((p(r)!.actualHours! - p(r)!.workingHours!) * 10) / 10 : '',
    p(r)?.completedAt ? formatDateTimeIST(p(r)!.completedAt!) : '',
    p(r)?.closedAt ? formatDateTimeIST(p(r)!.closedAt!) : '',
    (p(r)?.patents ?? []).join('; '),
                          ...
                          <td colSpan={6} className="px-3 py-3">
                            {r.project ? (
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-2 text-xs">
                                ...
                                {/* Completion record — only meaningful once the work has been delivered. */}
                                <div><span className="text-gray-400">Delivered to client</span><p className="text-gray-800">{r.project.clientDeliveryDate ? formatDateTimeIST(r.project.clientDeliveryDate) : '—'}</p></div>
                                <div><span className="text-gray-400">Working hours</span><p className="text-gray-800">{r.project.workingHours != null ? `${r.project.workingHours}h` : '—'}</p></div>
                                <div><span className="text-gray-400">Actual hours</span><p className="text-gray-800">{r.project.actualHours != null ? `${r.project.actualHours}h` : '—'}</p></div>
