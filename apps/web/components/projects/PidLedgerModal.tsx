...
/** Build + download a CSV of the (filtered) ledger rows. Opens directly in Excel. */
function exportCsv(rows: PidLedgerEntry[]) {
  const header = ['PID', 'Status', 'Project', 'Type', 'Deadline', 'Members', 'Generated by', 'Created', 'Resolved'];
  const body = rows.map(r => [
    r.pid, statusLabel(r.status), r.project?.title ?? '', r.project?.type ?? '',
    r.project?.dueDate ? formatDate(r.project.dueDate) : '',
    (r.project?.members ?? []).map(m => `${m.name} (${m.role})`).join('; '),
