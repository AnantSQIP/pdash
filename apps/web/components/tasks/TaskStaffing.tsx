...
const rowOf = (a: NonNullable<ApiTask['assignees']>[number]): Row => ({
  userId: a.userId,
  hours: a.estimatedHours != null ? String(a.estimatedHours) : '',
  start: dueOf(a.startDate),
  due: dueOf(a.dueDate),
  perDay: a.hoursPerDay != null ? String(a.hoursPerDay) : '',
});
/** An empty seat — spelled once, because a Row gaining a field and a literal not gaining it is
 *  how the daily ceiling came to be dropped on save in the first place. */
const EMPTY_ROW: Row = { userId: '', hours: '', start: '', due: '', perDay: '' };
const rowsFor = (task: ApiTask, role: TaskRole): Row[] =>
  (task.assignees ?? []).filter(a => a.role === role).map(rowOf);
...
const pmOf = (task: ApiTask, defaultManagerId?: string | null): Row => {
  ...
  if (pm) return rowOf(pm);
  return { ...EMPTY_ROW, userId: defaultManagerId ?? '' };
};

/**
 * Role-based task staffing: one Project Manager, many Reviewers, many Analysts. Per-person hours,
 * start, deadline and daily ceiling are all OPTIONAL. The SAME person may hold more than one role.
 * The task's total estimate is the sum of hours, saved server-side.
 *
 * Every field a seat carries has to appear here, because saving REPLACES the whole staffing list:
 * a field this form does not know about is a field the next save deletes. The start date and the
 * hours-a-day ceiling — the two things allocation from the capacity board sets — were exactly
 * that, so "2h a day from Monday" lasted until somebody opened this tab and pressed Save.
 */
export function TaskStaffing({ task, readOnly, canAssign, defaultManagerId, onSaved }: {
  ...
  function build(): StaffingEntry[] | string {
    ...
    const add = (userId: string, role: TaskRole, hoursStr: string, start: string, due: string, perDayStr: string): string | null => {
      ...
      // Caught here as well as on the server, so the person gets the answer in the form they are
      // filling in rather than as a failed save.
      if (start && due && start > due) return 'A start date cannot be after the deadline for the same person.';
      const capText = perDayStr.trim();
      const cap = capText === '' ? null : parseFloat(capText);
      if (cap !== null && (!Number.isFinite(cap) || cap < 0)) return 'Hours a day cannot be negative.';
      if (cap !== null && cap > 24) return 'Hours a day cannot be more than 24.';
      out.push({ userId, role, estimatedHours: h, startDate: start || null, dueDate: due || null, hoursPerDay: cap });
      return null;
    };
    for (const [uid, role, hrs, start, due, perDay] of ([[pm.userId, 'PM', pm.hours, pm.start, pm.due, pm.perDay]] as [string, TaskRole, string, string, string, string][])
      .concat(reviewers.map(r => [r.userId, 'REVIEWER', r.hours, r.start, r.due, r.perDay]))
      .concat(analysts.map(r => [r.userId, 'ANALYST', r.hours, r.start, r.due, r.perDay]))) {
      const err = add(uid, role, hrs, start, due, perDay);
      if (err) return err;
    }
  ...
  const PersonRow = ({ row, onChange, onRemove }: { row: Row; onChange: (r: Row) => void; onRemove?: () => void }) => {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        {/* Start first, then deadline — the order the work happens in, and the order that makes
            "from … until …" readable across the row. */}
        <input
          type="date" value={row.start} disabled={!editable} max={row.due || undefined}
          title="When this person starts. Leave blank to spread their hours up to the deadline."
        ...
        />
        <input
          type="date" value={row.due} disabled={!editable} min={row.start || undefined} title="Deadline for this person"
          onChange={e => onChange({ ...row, due: e.target.value })}
          className="w-36 shrink-0 px-2 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-brand-500 disabled:bg-gray-50 disabled:text-gray-400"
        />
        {/* The daily ceiling: how much of a day this job may take once it starts. Two hours a day
            for a week is a different plan from eight hours on Monday, and the total estimate alone
            cannot tell them apart. Blank means "as much of the day as is free". */}
        <div className="relative w-20 shrink-0">
          <input
            type="number" min="0" max="24" step="0.5" value={row.perDay} disabled={!editable}
            onChange={e => onChange({ ...row, perDay: e.target.value })}
            placeholder="all"
            title="Most hours a day this person spends on this task. Blank fills whatever of the day is free."
          ...
          />
          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] text-gray-400">/d</span>
        </div>
        {editable && onRemove && (
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <p className="text-[11px] text-gray-400">
        Hours, start, deadline and hours a day are optional per person, and a person can hold more
        than one role. Give someone a start date and their hours are placed on the capacity board
        from that day; leave it blank and they are spread evenly up to the deadline. Hours a day
        caps how much of each day this task may take — 2/d puts eight hours across four days.
      </p>
      {readOnly && (
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          ...
          {editable && (
            <button type="button" onClick={() => setReviewers(r => [...r, { ...EMPTY_ROW }])}
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          ...
          {editable && (
            <button type="button" onClick={() => setAnalysts(a => [...a, { ...EMPTY_ROW }])}
