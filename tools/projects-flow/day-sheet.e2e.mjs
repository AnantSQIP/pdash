...
(async () => {
  ...
  // FIXTURE (calendar), not behaviour: on a weekend or a holiday nothing is planned for the day
  // itself — the board places no work on a day with no hours — so fall back to the next planned
  // row rather than fail on the calendar. Same fix as tools/day-sheet.e2e.mjs (2d837dd).
