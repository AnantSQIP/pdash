...
(async () => {
  ...
  // FIXTURE, not behaviour: the "ordinary employee" is picked by what they may do, not by name.
  // ajay.sharma, who this suite was written against, became a Senior Consultant — with oversight
  // of every matter — and the walled-matter setup below then found nothing. EMPLOYEE, when set, is
  // tried first, and still has to pass the same check. The CLIENTS copy carries the same change.
  let emp = null;
  for (const email of [process.env.EMPLOYEE, 'aman.sharma@squarkip.com', 'drishti.jain@squarkip.com', 'rajesh.joshi@squarkip.com', 'ajay.sharma@squarkip.com'].filter(Boolean)) {
    const s = sess();
